        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4">
          <div ref={modalRef} className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 sm:p-8">
            <div className="flex items-center justify-between mb-5 pb-4 border-b border-slate-100">
              ...
              <button type="button" onClick={() => setInvoiceModal(false)} className="text-slate-400 hover:text-slate-600 cursor-pointer"><X className="w-5 h-5" /></button>
            </div>
            <form onSubmit={(e) => { e.preventDefault(); if (!invMember.trim() || !invAmount.trim()) { showToast('Please fill all fields.'); return; } showToast(`Invoice created for ${invMember} – ${invAmount}`); setInvoiceModal(false); setInvMember(''); setInvAmount(''); }} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1.5">Member Name</label>
                <input type="text" required value={invMember} onChange={(e) => setInvMember(e.target.value)} placeholder="Enter member name" className="w-full border border-slate-200 focus:border-blue-900 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-900" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1.5">Amount (USD)</label>
                <input type="text" required value={invAmount} onChange={(e) => setInvAmount(e.target.value)} placeholder="e.g. $1,200.00" className="w-full border border-slate-200 focus:border-blue-900 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-blue-900" />
              </div>
              <div>
              ...
              <div className="flex gap-3 pt-2 justify-end">
                <button type="button" onClick={() => setInvoiceModal(false)} className="px-4 py-2 text-sm font-semibold text-slate-500 hover:text-slate-700 cursor-pointer rounded-xl hover:bg-slate-50">Cancel</button>
      ...
      <main className="flex-grow pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-xs hover:shadow-md transition-shadow">
            ...
            <p className="text-3xl font-black text-slate-900 tracking-tight">$42,850.00</p>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-xs hover:shadow-md transition-shadow">
            ...
            <p className="text-3xl font-black text-slate-900 tracking-tight">$8,240.00</p>
            <p className="mt-3 text-xs font-semibold text-slate-400">14 invoices awaiting payment</p>
          </div>

          {/* Card 3: Overdue Payments */}
          <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-xs hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Overdue Payments</span>
              <div className="w-9 h-9 rounded-xl bg-rose-50 flex items-center justify-center">
                <AlertTriangle className="w-4.5 h-4.5 text-rose-600" />
              </div>
            </div>
            <p className="text-3xl font-black text-slate-900 tracking-tight">$2,150.00</p>
            <div className="mt-3 flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8 bg-white border border-slate-100 rounded-2xl p-6 shadow-xs">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-base font-bold text-slate-900">Payment Trends</h2>
              <div className="relative" ref={trendRef}>
                <button type="button" onClick={() => setTrendOpen((o) => !o)}
                  className="flex items-center gap-2 border border-slate-200 hover:border-slate-300 bg-white hover:bg-slate-50 text-slate-600 text-xs font-bold px-3.5 py-2 rounded-xl cursor-pointer transition-all">
                  {trendLabel}
                  <ChevronDown className="w-3.5 h-3.5 text-slate-400" />
                </button>
                {trendOpen && (
                  <div className="absolute right-0 mt-1.5 w-44 bg-white border border-slate-100 rounded-xl shadow-lg z-30 p-1">
                    {['Last 3 Months', 'Last 6 Months', 'Last 12 Months', 'This Year'].map((opt) => (
                      <button key={opt} type="button" onClick={() => { setTrendLabel(opt); setTrendOpen(false); showToast(`Trends filtered: ${opt}`); }}
                        className={`w-full text-left px-3 py-2 text-xs font-semibold rounded-lg transition-all cursor-pointer ${trendLabel === opt ? 'bg-blue-50 text-blue-900' : 'text-slate-600 hover:bg-slate-50'}`}>
                        {opt}
                      </button>
                    ))}
                  </div>
            ...
            {/* Bar chart area */}
            <div className="relative h-52 flex items-end justify-between gap-4 px-2 pt-4 border-b border-slate-100">
          ...
          {/* System Health */}
          <div className="lg:col-span-4 bg-white border border-slate-100 rounded-2xl p-6 shadow-xs flex flex-col gap-6">
            <h2 className="text-base font-bold text-slate-900">System Health</h2>

            {/* Occupancy Rate */}
            <div>
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-slate-600">Occupancy Rate</span>
              ...
              <p className="text-[10px] text-slate-400 font-semibold mt-1.5">46 of 50 units occupied</p>
            </div>

            {/* Smart Locks status */}
            <div className="bg-slate-50/80 border border-slate-100 rounded-xl px-4 py-3.5 flex items-center gap-3">
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-pulse shrink-0" />
              <div>
                <p className="text-xs font-bold text-slate-800">Smart Locks Online</p>
                <p className="text-[11px] text-slate-400 font-semibold mt-0.5">42 Active Hubs</p>
              </div>
            ...
            <div className="bg-slate-50/80 border border-slate-100 rounded-xl px-4 py-3.5 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-bold text-slate-800">Security System</p>
                <p className="text-[11px] text-slate-400 font-semibold mt-0.5">All sensors normal</p>
              </div>
            ...
            <button type="button" onClick={() => showToast('Opening detailed audit report…')}
              className="w-full border border-slate-200 hover:border-blue-900 bg-white hover:bg-blue-50 text-slate-700 hover:text-blue-900 text-sm font-bold py-2.5 rounded-xl cursor-pointer transition-all">
              View Detailed Audit
            </button>
          </div>
        ...
        <div className="bg-white border border-slate-100 rounded-2xl shadow-xs overflow-hidden">
                  ...
                  <tr key={inv.id} className="hover:bg-slate-50/60 transition-colors group">
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2.5">
                        <img src={inv.avatar} alt={inv.member} className="w-8 h-8 rounded-full object-cover border border-slate-100 shrink-0" />
                        <span className="text-sm font-semibold text-slate-800 whitespace-nowrap">{inv.member}</span>
                      </div>
                    </td>

                    {/* Amount */}
                    <td className="px-5 py-4">
                      <span className="text-sm font-extrabold text-slate-900">{inv.amount}</span>
                    </td>

                    {/* Due Date */}
                    <td className="px-5 py-4">
                      <span className={`text-sm font-semibold ${inv.dueDateRed ? 'text-rose-600 font-bold' : 'text-slate-600'}`}>
                        {inv.dueDate}
                      </span>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                        <button type="button" title="View invoice" onClick={() => showToast(`Opening invoice ${inv.id}…`)}
                          className="p-1.5 text-slate-400 hover:text-blue-900 hover:bg-blue-50 rounded-lg cursor-pointer transition-all">
          ...
          <div className="border-t border-slate-100 px-5 py-4 flex items-center justify-between gap-3">
            <p className="text-xs font-semibold text-slate-400">Showing 1–10 of 124 invoices</p>
            <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-5">
            {['Support', 'Privacy Policy', 'Terms of Service', 'Contact Us'].map((link) => (
              <a key={link} href="#" onClick={(e) => { e.preventDefault(); showToast(`Opening ${link}…`); }}
                className="text-xs font-semibold text-slate-500 hover:text-blue-900 transition-colors underline-offset-2 hover:underline">
                {link}
              </a>
            ))}
          </div>
